...
export const myPlaylists = (state = DEFAULT_STATE, action) => {
    switch (action.type) {
        ...
        case REORDER_TRACKS_IN_PLAYLIST:
            const { dragIndex, hoverIndex, playlistId } = action;
            playlists = Array.from(state.playlists);

            let playlistToMutate = R.find(R.propEq('id', playlistId))(
                playlists
            );
            let { tracks: { list: tracklist } } = playlistToMutate;

            let dragTrack = tracklist[dragIndex];
            tracklist[dragIndex] = tracklist[hoverIndex];
            tracklist[hoverIndex] = dragTrack;
